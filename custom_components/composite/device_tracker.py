...
from homeassistant.components.device_tracker import (
    ATTR_BATTERY, ATTR_SOURCE_TYPE, PLATFORM_SCHEMA, SOURCE_TYPE_ROUTER)
from homeassistant.const import (
    ATTR_GPS_ACCURACY, ATTR_LATITUDE, ATTR_LONGITUDE, CONF_ENTITY_ID,
    CONF_NAME, EVENT_HOMEASSISTANT_START, STATE_NOT_HOME)
...
class CompositeScanner:
    ...
    def _update_info(self, entity_id, old_state, new_state):
        ...
        if source_type == SOURCE_TYPE_ROUTER and new_state.state == STATE_NOT_HOME:
            _LOGGER.info("Skipping update from '{}' since source type is '{}' and new state is '{}'.".format(entity_id, source_type, new_state.state))
            return
